...
#define cout io
// </fastio.hpp>
// <range_queries/simd_segtree.hpp>
#include <array>
/**
...
struct SIMDSegTree {
  ...
  T sum(int i) {
    T s = 0;
    for (int h = 0; h != H; ++h) {
      if (h + 1 != H) prefetch(h + 1, i >> branching_bits);
...
int main() {
  ...
  SIMDSegTree<long long, 1 << 18, decltype(add)> st(add);
